...
import { StatusInlineChange } from "@/components/StatusInlineChange";
import { MessageCircle, ArrowLeft, Calendar, Hash, Layers, Radio } from "lucide-react";
...
export default async function LeadDetailPage({
  ...
  const fields = [
    { label: "Procedimento", value: lead.procedimento ?? "—", icon: Layers },
    { label: "Canal", value: lead.canal, icon: Radio },
    { label: "Segmento", value: lead.segmento, icon: Hash },
    { label: "Mês de Referência", value: lead.mes_referencia ?? "—", icon: Calendar },
    { label: "Data de Contato", value: lead.data_contato ?? "—", icon: Calendar },
    {
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-km-surface rounded-xl border border-km-border p-6">
        <h2 className="text-sm font-semibold text-gray-700 mb-4">
          Interações da Luísa{" "}
          <span className="text-gray-400 font-normal">({interacoes.length})</span>
        </h2>
          ...
          <ol className="relative border-l border-km-border space-y-4 ml-2">
            {interacoes.map((i) => (
              <li key={i.id} className="pl-5">
                ...
                {i.mensagem && (
                  <p className="text-sm text-gray-700 mt-1">{i.mensagem}</p>
                )}
              </li>
            ))}
          </ol>
